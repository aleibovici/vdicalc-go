...
func init() {
	...
	hostResults.Hostclockusedlimit = "5.5"                     // Typical modern CPU turbo ceiling (soft warning)
	...
	for key, values := range fullData {
		...
		case "hostresultsclockused":
			...
			if functions.StrtoFloat64(values.(string)) > functions.StrtoFloat64(hostResults.Hostclockusedlimit) {
				error := config.ErrorConfiguration{Code: "Warning: ", Description: "Host CPU (GHz) above typical turbo limit. (max≈5.5)"}
				errorList.Error = append(errorList.Error, error)
				return errorList, true
			}
